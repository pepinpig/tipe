...
# Ouvrir le fichier en écriture et stocker les dimensions + pixels sous forme de matrice
with open(output_path, 'w') as f:
    f.write(f"{width} {height}\n")  # Écrire la hauteur et la largeur (hauteur = nombre de lignes, largeur = nombre de colonnes)
    for row in image:
        row_str = ' '.join(map(str, row))  # Convertir chaque pixel en string
        f.write(row_str)  # Écrire chaque ligne de pixels dans le fichier
