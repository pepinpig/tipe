...
from sympy import symbols, diff, lambdify, sqrt, Piecewise
...
# Definir x et y comme des variables mathématiques
x, y = symbols('x y')

# Position initiale et finale de la voiture + obstacles
x0, y0 = 3, 3
XG = 90
YG = 90
obstacle_list = [[60, 80, 10, 1500, 500], [10, 60, 5, 800, 300], [80, 40, 3, 500, 100]]  # Xobs, Yobs, obs_size, η, R0

#Calcul de la fonction potentiel de l'obstacle
...
# Specify the learning rate and momentum coefficient
lr = 100
momentum =0.0
...
# on met à jour la trajectoire à chaque seconde
for i in range(1000):
    # Compute the gradients
    grad_x_val = grad_x(x0, y0)
    grad_y_val = grad_y(x0, y0)
    
    # Update velocities
    v_x = momentum * v_x - lr * grad_x_val
    v_y = momentum * v_y - lr * grad_y_val
    
    # Update the x and y values using the velocities
    x0 += v_x
    y0 += v_y
...
ax2.set_title('Descente de gradient avec inertie')
